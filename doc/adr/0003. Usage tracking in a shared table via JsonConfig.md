...
* `globaljsonlinks` matches up the smaller wiki IDs with the `Data:` target page IDs from the other two tables via ns/title text of the linking page. This should be quite compact for widely used Data: pages, and makes it relatively easy to get a list of remote wikis to target (additional complexity: have to determinie namespace text forms).
