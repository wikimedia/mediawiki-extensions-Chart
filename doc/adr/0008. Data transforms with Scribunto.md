...
Initial version will use a transform funciton reference and arguments encapsulated in the `Data:*.chart` JSON, invoked as normal:

```mediawiki
{{#chart:Weekly average temperatures (C).chart
...
with a `Data:Weekly average temperatures (C).chart` JSON referencing the raw data:
...
and `Data:Weekly average temperatures (F).chart` with a transformed version:
...
The sample data set `Data:Sample weekly temperature dataset.tab`:
...
The `action=jsontransform` request will include a `jtmodule`, `jtfunction`, and `jtargs` parameters, where `jtmodule` and `jtfunction` are individual strings and `args` is a multi-string set allowing duplicates, meant to contain `key=value` pairs. This is meant to be compatible with specifying arg overrides to the `{{#chart:}}` parser function in the future.
